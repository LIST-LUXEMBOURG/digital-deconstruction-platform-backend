...
export function WithParam<TBase extends Constructor>(Base: TBase) {
    class Param extends WithProjectId(Base) {
        @ApiProperty({
            type: String,
            required: true,
            description: 'The element type uid',
        })
        @IsNotEmpty()
        @IsUUID()
...
export class CreateInventoryPropertyParamsDto extends WithParam(class { }) { }
export class CreateInventoryPropertyBodyDto extends WithBody(class { }) { }
export class CreateInventoryPropertyDto extends WithBody(
