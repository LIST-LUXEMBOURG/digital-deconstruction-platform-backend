...
function WithParams<TBase extends Constructor>(Base: TBase) {
    class Params extends WithProjectId(Base) {
        @ApiProperty({
            type: String,
            required: true,
            description: 'The material type identifier',
        })
        @IsUUID()
        @IsNotEmpty()
        uid: string;
...
export class AddInventoryMaterialTypeCircularityBodyDto extends WithParams(class { }) { }
export class AddInventoryMaterialTypeCircularityParamsDto extends WithParams(class { }) { }
export class AddInventoryMaterialTypeCircularityDto extends WithParams(Authorization) { }